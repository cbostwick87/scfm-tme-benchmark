"""Construct and persist three-way leakage-controlled splits under S1/S2/S3.
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description='Construct and persist three-way leakage-controlled splits under S1/S2/S3.')
    ap.add_argument("--config", default="configs/default.yaml")
    ap.add_argument("--force", action="store_true", help="recompute even if cached output exists")
    args = ap.parse_args(argv)
    cfg = config.load(args.config)
    raise NotImplementedError(
        "stage s03_splits is not implemented yet; it is scaffolded so the pipeline shape "
