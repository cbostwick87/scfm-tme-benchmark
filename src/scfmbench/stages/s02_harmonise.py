"""QC, gene-symbol harmonisation, label taxonomy -> T4, pairwise gene-overlap matrix, stratified subsample.
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description='QC, gene-symbol harmonisation, label taxonomy -> T4, pairwise gene-overlap matrix, stratified subsample.')
    ap.add_argument("--config", default="configs/default.yaml")
    ap.add_argument("--force", action="store_true", help="recompute even if cached output exists")
    args = ap.parse_args(argv)
    cfg = config.load(args.config)
    raise NotImplementedError(
        "stage s02_harmonise is not implemented yet; it is scaffolded so the pipeline shape "
