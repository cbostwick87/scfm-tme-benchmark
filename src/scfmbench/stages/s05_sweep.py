"""Run the design grid with an identical LR head; append incrementally to tidy table T2.
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description='Run the design grid with an identical LR head; append incrementally to tidy table T2.')
    ap.add_argument("--config", default="configs/default.yaml")
    ap.add_argument("--force", action="store_true", help="recompute even if cached output exists")
    args = ap.parse_args(argv)
    cfg = config.load(args.config)
    raise NotImplementedError(
        "stage s05_sweep is not implemented yet; it is scaffolded so the pipeline shape "
