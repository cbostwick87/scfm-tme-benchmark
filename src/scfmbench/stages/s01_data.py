"""Acquire raw data, resolve the duplicate-accession hazard, hash every file, build T1.
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description='Acquire raw data, resolve the duplicate-accession hazard, hash every file, build T1.')
    ap.add_argument("--config", default="configs/default.yaml")
    ap.add_argument("--force", action="store_true", help="recompute even if cached output exists")
    args = ap.parse_args(argv)
    cfg = config.load(args.config)
    raise NotImplementedError(
        "stage s01_data is not implemented yet; it is scaffolded so the pipeline shape "
